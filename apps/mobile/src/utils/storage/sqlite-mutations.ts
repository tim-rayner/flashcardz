/**
 * Generic insert/delete/read helpers used by OnboardStore. Table and column
...
export async function getRow<T extends TableName>(
  ...
  return db.getFirstAsync<TableRowMap[T]>(
...
export async function upsertRow<T extends TableName>(
  ...
  const updateColumns = columns.filter(
    (column) => !(keyColumns as readonly string[]).includes(column),
  );
